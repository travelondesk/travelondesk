...
<form class="form-signin" method="POST">
    {% csrf_token %}
        <p class="required"><label for="id_username">{% trans "Username:" %}</label>
		{{form.username}}</p>
		<strong>{{form.errors.username}}</strong>
        <p class="required"><label for="id_password">{% trans "Password:" %}</label>
        {{form.password}}</p>
		<strong>{{form.errors.password}}</strong>
		<strong>{{form.non_field_errors}}</strong>
        <button class="btn btn-lg btn-primary btn-block" type="submit">{% trans "Enter" %}</button>
